...
const Card = ({ children }: { children: any }) => <div className={styles.Card}>{children}</div>
...
export const CandidateCard = ({ useDiD, candidate }: { useDiD: boolean; candidate: Candidate }) => {
  ...
  async function handleVote() {
    ...
    if (vote) {
      if (useDiD) {
        const connected = await connect().catch(errorHandler)
      }

      vote()?.then(successHandler).catch(errorHandler)
    }
  }
  ...
  async function errorHandler(e: Error) {
    ...
    if (
      e.message.startsWith("The identity is not part of the group") ||
      e.message.startsWith("not in group")
    )
      return voteDialog.open({
        ...errorDialog,
        children: (
    ...
    <>
      ...
      <Card>
        ...
        <div className={styles.CandidateCard + " " + (useDiD ? styles.useDiD : "")}>
          {elected && (
            <div className={styles.elected}>
              <img src={electedImage.src} alt="elected" />
            </div>
          )}
          {vote ? (
            <Button
              onClick={() => voteDialog.open(confirmDialog)}
              text="投票給我"
              variant="primary"
            />
          ) : (
