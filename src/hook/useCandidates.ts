...
export default function useCandidates({ useDiD }: { useDiD: boolean }) {
  ...
  useEffect(() => {
    if (announced) fetchVoteCounts().then(setVoteCounts)
  }, [])

  return CandidatesInfo.map((candidate) => ({
    ...candidate,
    voteCount:
      announced && voteCounts[candidate.index]
        ? voteCounts[candidate.index] + (voted == candidate.index ? 1 : 0)
        : undefined,
    handleVote: !isVotePeriod
      ? undefined
      : useDiD
        ? () => didVote(candidate.index)
        : () => liveVote(candidate.index),
    elected:
      announced && voteCounts[candidate.index]
        ? voteCounts[candidate.index] === Math.max(...voteCounts)
        : undefined
  }))
