...
export async function POST(request: Request) {
  try {
    ...
    if (!isVotePeriod) return Response.json({ message: "election ended" }, { status: 403 })

    const requestBody = await parseRequest(request)
    if (!requestBody) return Response.json({ message: "invalid request body" }, { status: 403 })

    const { candidateId } = requestBody
    if (candidateId > 3) return Response.json({ message: "invalid candidateId" }, { status: 403 })
