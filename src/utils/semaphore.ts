...
import { CommitmentsDto, SEMAPHORE_GROUP_DEPTH, SEMAPHORE_GROUP_ID } from "@tw-did/core/src"
...
export default async function generateSemaphoreProof(candidateId: number) {
  ...
  const fullProof = await generateProof(identity, group, proposalId, candidateId)
